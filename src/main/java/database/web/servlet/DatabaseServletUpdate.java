...
public class DatabaseServletUpdate extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public DatabaseServletUpdate() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(method.equals("search"))
		{
			...
			if(database.getDatabase()!=null){
				...
				request.getRequestDispatcher("/jsps/database/database_update_output.jsp").forward(request, response);
...
//			form.setEmail(info.get(3));
			form.setDatabase(request.getParameter("database"));

			try {
				databasedao.update(form);
